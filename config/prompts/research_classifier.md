...
A topic does not have to populate every bucket. **A documents-only
plan is legal** — a topic like "OFAC SDN list updates" might
legitimately have zero metrics and zero entity kinds and just one
document source. The plan should populate the buckets the topic
*actually involves*, and leave the others empty.
...
- **Geographic scope (`geographic_scope`)** — prefer **ISO 3166-1
  alpha-2** country codes when applicable: `US`, `CN`, `HU`, `BR`.
  When the scope is a region rather than a country, use a
  lowercase descriptor: `east_asia`, `lithium_triangle`, `asean`,
  `eu_27`. **Do not write "United States" or "the EU"** — write
  `US` or `eu_27`.
...
- `geographic_scope` uses ISO alpha-2 codes, not country names.
