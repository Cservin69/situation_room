...
// ---------------------------------------------------------------------------
// RecordCountsDto — plan ↔ record satisfaction view (Session 14, P2)
// ---------------------------------------------------------------------------

/// Record counts by type for a given plan, derived by joining the
/// plan's topic tags against `record_subjects_topics`. A count of
/// zero for a type means records of that type were expected (per
/// the plan's `RecordExpectations`) but haven't landed yet —
/// either because a fetch run hasn't happened, or because all
/// recipes for that expectation failed.
///
/// The frontend renders these alongside the six bucket panels so
/// the user can see at a glance which expectations are satisfied.
/// The satisfaction check is coarse: "did *anything* land with this
/// plan's topics?" not "did the *right* thing land?" — that's the
/// Class-D distinction ADR 0012 calls out and defers.
///
/// Fields are `u32` (not `u64`) so ts-rs emits `number`, not
/// `bigint`. Record counts fit comfortably within u32.
#[derive(Debug, Clone, Serialize, Deserialize, TS)]
#[ts(export, export_to = "../../../apps/desktop/src/lib/api/types/")]
pub struct RecordCountsDto {
    ...
    pub documents: u32,
    pub assertions: u32,
    /// Convenience total: sum of all six. Lets the frontend render a
    /// single "N records" summary without repeating the arithmetic.
    pub total: u32,
}

impl From<situation_room_storage::RecordCountsForPlan> for RecordCountsDto {
    fn from(c: situation_room_storage::RecordCountsForPlan) -> Self {
        let total = c.total();
        Self {
            observations: c.observations as u32,
            events:       c.events       as u32,
            entities:     c.entities     as u32,
            relations:    c.relations    as u32,
            documents:    c.documents    as u32,
            assertions:   c.assertions   as u32,
            total:        total           as u32,
        }
    }
}
