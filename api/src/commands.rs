...
/// Return record counts by type for all records sharing at least one
/// topic tag with the given plan. This is the plan ↔ record
/// satisfaction view (Session 14, P2 per ADR 0012 §"Class D blind
/// spot").
///
/// The join is topic-based: every record produced by the executor
/// inherits the plan's `topic_tags` during normalization (ADR 0007
/// §"Normalization stage"). A count of zero for a type means no
/// records of that type have landed yet for this plan's topics.
///
/// The query is cheap (a set of `COUNT(DISTINCT)` over the topics
/// junction table) and safe to call frequently — the frontend may
/// call it after each `run_fetch_for_plan` to refresh the counts.
#[tauri::command]
pub async fn get_record_counts_for_plan(
    plan_id: String,
    state: tauri::State<'_, AppState>,
) -> Result<crate::types_export::RecordCountsDto, CommandError> {
    let parsed: Uuid = plan_id
        .parse()
        .map_err(|e: uuid::Error| CommandError::InvalidInput {
            field: "plan_id".into(),
    ...
    // Load the plan to get its topic tags. An unknown plan id is a
    // NotFound — the caller should only call this on a plan they've
    // already fetched via get_plan.
    ...
    .ok_or_else(|| CommandError::NotFound { id: plan_id })?;

    let topic_strings: Vec<String> = plan
        .topic_tags
        .iter()
    ...
    let counts = state
        .store
        .record_counts_for_plan(&topic_strings)
        .map_err(CommandError::from)?;

    Ok(crate::types_export::RecordCountsDto::from(counts))
}
...
mod tests {
    ...
    #[test]
    fn record_counts_dto_serializes_with_number_fields() {
        // Guard the wire shape: all counts must serialize as JSON numbers
        // (not bigints / strings) so the frontend's TypeScript `number`
        // type works without coercion. This would fail if the DTO fields
        // were u64 and ts-rs had emitted `bigint` — serde_json would
        // serialize u64 as a number, but TypeScript would reject the
        // type at compile time. The u32 → number mapping is the
        // load-bearing contract.
        use crate::types_export::RecordCountsDto;
        use situation_room_storage::RecordCountsForPlan;

        let counts = RecordCountsForPlan {
            observations: 3,
            events:       1,
            entities:     0,
            relations:    0,
        ...
        let dto = RecordCountsDto::from(counts);
        let json = serde_json::to_string(&dto).unwrap();

        // All fields must be plain JSON numbers, not quoted strings.
        assert!(json.contains(r#""observations":3"#));
        assert!(json.contains(r#""events":1"#));
        assert!(json.contains(r#""entities":0"#));
        assert!(json.contains(r#""total":6"#));
        // Sanity: the From conversion computed total correctly.
