...
## Per-target authoring — your call is one of N targets on these bytes

You are reading this prompt because the runtime called you with
ONE target expectation against ONE pre-fetched document. The
runtime is also calling you, separately, for the OTHER target
expectations the plan declared — same bytes, different target —
up to ~6 calls per nominated source. Each call is independent.
Sibling targets succeeding does not help you; sibling targets
declining does not excuse you. Your job is THIS target, against
THESE bytes.

**The lever this section names.** The most common failure mode
in live runs is: the LLM sees one nominated source that
plausibly carries an observation metric (a financial-data
endpoint, a statistical-agency time-series, a price API),
authors that one obs binding cleanly, and then declines the
plan's remaining slots (a second obs metric on the same JSON, a
named entity that appears in the response, an event-type the
source's archive index lists) because they "feel" like a
different source's job. They are not. Every target gets its own
shot against these bytes. Decline only when THIS target, against
THESE bytes, genuinely fails the four-condition checklist
below — not because a sibling target succeeded or because
another L1 nomination is "probably better for this one."

**Plausible source-class fits — closed-vocabulary, structural.**
When the target's plausible source class matches the class of
bytes you have, AUTHORING is the default, not declining. Map by
shape, not by host:

- `observation_metric` target → time-series JSON, indicator
  catalog, statistical-agency CSV, OHLC / quote / chart endpoint,
  per-instrument financial-data response. If the response carries
  ANY of the plan's observation_metrics as a leaf — even one
  embedded among many — the metric you've been given a target
  for might also be there. Walk the response shape; do not
  decline by analogy ("this looks like a price endpoint so I'll
  only author price"). Endpoints in this class commonly return
  composite responses with several metrics co-located on the same
  instrument (close + volume + market_cap + dividends as fields
  in one JSON object, or sibling rows in one CSV).

- `event_type` target → filings index, press-release archive,
  regulatory action log, news topic feed, disclosure portal,
  earnings-calendar page. These are LISTING-shaped — the right
  authoring move is an `iterator` recipe per the "Iterating over
  listings" section, or a follow-the-link author per the
  Diagnosis-driven re-authoring section above. The bytes being
  "a list of headlines" or "a table of filings" is not a decline
  reason for an `event_type` target; it is the normal shape.

- `entity_kind` target → roster, directory, member-of page,
  signatories list, named-participants table, leaderboard. Same
  shape as the v1.25 worked example: iterator over the rows,
  `entity_id` as a stable per-row literal or extracted slug,
  `kind` from the plan, `canonical_name` from the per-row leaf.
  A page listing N named actors is the normal authoring surface,
  not a decline.

- `relation_kind` target → mentions feed, ownership table,
  transactions log, citations index, affiliations roster. Same
  multi-leaf relation pattern as the v1.20/v1.25 examples.

**Same bytes, different target — these are NOT decline reasons.**
- "I already authored a different obs metric on this URL." Each
  target is its own author call. Author this one too.
- "Sibling target on this URL declined." Sibling declines do not
  bind your decision; your target may have a different shape on
  the same bytes.
- "This URL looks like a price endpoint, my target is an event."
  The URL was proposed for the *nomination's* description, not
  for your specific target. If the bytes happen to also carry
  event-shaped data (a "news" section in a chart response, an
  "events" array, etc.), author. If not, decline — but the
  reason is "these bytes carry no event-shaped leaf for this
  type," not "this URL was meant for prices."
- "Another L1 nomination is probably better for this target."
  Other nominations get their own author calls. You are not
  routing across nominations; you are authoring against the bytes
  in front of you. If those bytes carry the target slot's record
  shape, author here too — the runtime de-duplicates downstream.

**When honest decline is still right.** When the bytes truly do
not carry the target slot's record shape — e.g. an obs-metric
target whose required `value` field has no numeric leaf anywhere
in the response, or an event_type target on a static "about us"
page with no list of items — decline using the four-condition
checklist below. The point of this section is to surface the
common wrong-decline patterns, not to suppress all declines.
...
- **v1.26** (2026-05-18) — Session 101, per-target authoring
  discipline. Adds a new section **Per-target authoring — your
  call is one of N targets on these bytes** immediately before
  *Coverage discipline*. The new prose names the architectural
  fact (one nomination → up to ~6 target-narrowed author calls
  against the same prefetched bytes, each call independent) and
  the failure mode the section is designed to suppress (declining
  this target because a sibling target on the same URL already
  authored, or because the URL "feels" like a different target's
  job). Adds a closed-vocabulary plausible-source-class map for
  each of the four record-typed buckets — observation_metric,
  event_type, entity_kind, relation_kind — naming the structural
  shapes each typically lives on (time-series API / filings
  index / roster listing / mentions feed) without naming any
  host. Adds four explicit "NOT decline reasons" tied to the
  per-target architecture. Closes with "when honest decline is
  still right" pointing back to the existing four-condition
  checklist so this section adds aggression, not laxity.

  No output-contract change. No schema change. No Rust changes.
  Existing recipes are unaffected; the new prose targets fresh
  authoring calls only.

  Motivation: Sn-101 diagnosis on the TESLA stock price plan
  (`019e3a75-62c9-72f1-bb5c-48e6cb54f3c7`) — L1 emitted 4 source
  classes via `document_sources` (SEC EDGAR, IR PDFs, financial-
  data platform, news wire). The executor's dispatch
  (`build_target_expectations` + per-target author calls, ADR
  0018 + Session 47) reached all of them. The financial-data
  nomination's URL locked on Yahoo Finance; obs[0]
  (closing_price) and obs[1] (trading_volume) authored cleanly
  against the chart JSON; obs[2] (market_cap) declined on the
  same bytes even though the Yahoo chart response also carries a
  market-cap field. The other 3 nominations declined at the
  nomination level — 4 expectation-level declines + 3
  nomination-level declines = 7 `decl·nom` pills on the
  dashboard, matching `fetch_run_outcomes`. Final coverage 2/8.
  The lever is not "race per slot" (already happens); it is
  "treat each target's call as independent and recognise when
  the bytes carry the slot's record shape."

  Closed-vocabulary discipline preserved: no host strings, no
  per-publisher rules, no per-plan carve-outs. Every shape named
  in the new section is structural (time-series, listing,
  roster, feed) and grounded in the four closed record types.
