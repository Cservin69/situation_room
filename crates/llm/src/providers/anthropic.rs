...
fn map_http_err(e: HttpError) -> LlmError {
    match e {
        ...
        HttpError::Status(429) => LlmError::RateLimited {
            // Anthropic returns retry-after in headers;
            // SecureHttpClient doesn't surface response headers
            // today (carry-forward from Session 19+). Report 0 as
            // "unknown" — the router can apply its own backoff.
            // When the SecureHttpClient header-exposure followup
            // lands, this is the place to thread the real value
            // through. Same posture as xAI.
