...
import {
  ...
  getRecordCountsForPlan as apiGetRecordCounts,
  asCommandError,
} from '$lib/api/client';
...
interface PlansState {
  ...
  recipes: RecipeDto[];
  /**
   * Record counts by type for the selected plan — how many records
   * bearing this plan's topic tags have landed in storage. Null until
   * the first `refreshRecordCounts` call lands (i.e. until a plan is
   * selected). Zero counts mean the plan's expectations haven't been
   * satisfied yet by any fetch run.
   *
   * Drives `SatisfactionPanel.svelte`. Refreshed on plan selection and
   * after each successful `runFetch`.
   */
  recordCounts: RecordCountsDto | null;
...
export async function selectPlan(id: string): Promise<void> {
  ...
  try {
    ...
    // Load record counts in the background so the satisfaction panel
    // shows immediately without blocking the plan body render. Null
    // until this call lands — the panel renders a loading state.
    void refreshRecordCounts(id).catch(() => {});
  } catch (e) {
    plans.error = asCommandError(e);
  } finally {
    plans.loading = false;
...
export async function runFetch(): Promise<boolean> {
  ...
  try {
    ...
    // Refresh record counts — the run may have produced new records
    // and the satisfaction panel should reflect that immediately.
    void refreshRecordCounts(current.id).catch(() => {});
    return true;
  } catch (e) {
    plans.error = asCommandError(e);
    return false;
...
/**
 * Refresh the record counts for a plan. Pure read; called alongside
 * `selectPlan` and after each successful `runFetch`.
 *
 * Like `refreshFetchRuns`, this doesn't toggle any spinner. The
 * satisfaction panel renders its own null/loading state until the
 * call returns. Non-fatal on failure — a transient read error leaves
 * the prior counts in place rather than blanking the panel.
 */
export async function refreshRecordCounts(planId: string): Promise<void> {
  try {
    plans.recordCounts = await apiGetRecordCounts(planId);
  } catch (e) {
    // Non-fatal: same rationale as refreshFetchRuns / refreshRecipes.
    // Don't reset plans.recordCounts — preserving stale counts is
    // more useful than blanking the panel on a transient failure.
    plans.error = asCommandError(e);
  }
}
 *
