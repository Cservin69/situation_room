...
import type { RecordCountsDto } from './types/RecordCountsDto';
 ...
 * satisfaction view (Session 14, P2).
 *
 * Call after `run_fetch_for_plan` completes to refresh the counts,
 * or on plan selection to show the state of prior fetch runs.
 *
 * Returns zeros for plans that have never been fetched, or for plans
 * whose fetch runs produced no records.
 *
 * Throws `{ kind: 'not_found' }` if the plan id is unknown.
 */
export async function getRecordCountsForPlan(planId: string): Promise<RecordCountsDto> {
  return invoke<RecordCountsDto>('get_record_counts_for_plan', { planId });
}
