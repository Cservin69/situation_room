...
<script lang="ts">
  ...
  import SatisfactionPanel from '$components/panels/SatisfactionPanel.svelte';
...
<article class="plan">
  ...
  <!-- Plan ↔ record satisfaction view (Session 14, P2).
       Shows how many records of each type have landed for this plan.
       Renders nothing when no fetch has run and counts are all zero;
       shows a shimmer while the counts are loading; shows the pill
       row when any records exist. See SatisfactionPanel.svelte for
       the full rendering contract. -->
  <SatisfactionPanel
    counts={plans.recordCounts}
    hasRuns={plans.fetchRuns.length > 0}
  />
